-- Adaptation: factory isolation and tracked resources for executor reinitialization.
return function(ctx)
local scope = ctx.cleanup:scope()
local NativeInstance = Instance
local Instance = {new = function(...) return scope:add(NativeInstance.new(...)) end}
local NativeTweenService = game:GetService("TweenService")
local TrackedTweenService = {Create = function(_, ...) return scope:add(NativeTweenService:Create(...)) end}
--[[
...
local TweenService = TrackedTweenService
...
function Rain:destroy() scope:destroy() end
return Rain
end